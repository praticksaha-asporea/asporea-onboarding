...
import {
  ...
  Typography, Box, TextField, CircularProgress, Chip, Grid,
...
import { useTechnicalActionModal } from "./useTechnicalActionModal";
...
// ── Inline PDF / image upload local component layout ──
const BreakdownPdfUpload: React.FC<BreakdownPdfUploadProps> = ({ file, onChange }) => {
  ...
  return (
    <>
      ...
      >
        <input ref={inputRef} type="file" hidden accept={accept.join(",")} onChange={handleChange} />
        {file ? (
          <Box className="flex flex-col items-center text-center w-full gap-1">
            {isImage && previewUrl ? (
              <Box className="relative group/thumb w-full flex justify-center mb-1 cursor-pointer" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setPreviewOpen(true); }}>
              ...
              <Box className="w-10 h-10 rounded-full bg-green-50 border border-green-200 flex items-center justify-center mb-1 cursor-pointer hover:bg-green-100 transition-all relative group/pdf" onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (previewUrl) setPreviewOpen(true); }}>
            ...
            <Typography className="text-[12px] font-bold text-[var(--mui-palette-text-primary)] break-all px-2 max-w-full">{file.name}</Typography>
            <Typography className="text-[11px] text-[var(--mui-palette-text-secondary)]">Click card to change</Typography>
            <Typography onClick={handleClear} className="text-[11px] text-red-500 font-bold underline cursor-pointer hover:text-red-700 mt-0.5">Clear</Typography>
          </Box>
        ) : (
          <Box className="flex flex-col items-center text-center">
            ...
            <Typography className="text-xs font-semibold mb-1">Drop file here or <span className="text-[var(--mui-palette-primary-main)] font-extrabold">browse</span></Typography>
            <Typography className="text-[10px] text-gray-400 uppercase mt-1">PDF · JPG · PNG — max 5 MB</Typography>
          </Box>
        )}
      </Box>
      ...
      <Dialog open={previewOpen} onClose={() => setPreviewOpen(false)} maxWidth="md" fullWidth>
        <Box className="flex items-center justify-between px-5 py-3 border-b border-[var(--mui-palette-divider)]">
          <Typography variant="subtitle1" className="font-bold truncate max-w-[80%]">{file?.name}</Typography>
          <Box className="flex items-center gap-2">
            ...
            <Button size="small" variant="text" onClick={() => setPreviewOpen(false)}><i className="ri-close-line text-xl" /></Button>
          </Box>
        </Box>
  ...
  return (
    <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth PaperProps={{ className: "rounded-xl" }}>
      ...
      <DialogContent className="flex flex-col gap-5 pt-6 bg-[var(--mui-palette-primary)]">
        <Box className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Box className="bg-[var(--mui-palette-primary)] p-4 rounded-xl shadow-2xl">
            <Typography variant="subtitle2" className="text-[var(--mui-palette-primary)] text-[12px] uppercase tracking-wider">Candidate</Typography>
          ...
          <Box className="bg-[var(--mui-palette-primary)] p-4 rounded-xl shadow-2xl flex flex-col justify-between">
            <Box>
              <Typography variant="subtitle2" className="text-[var(--mui-palette-primary)] text-[12px] uppercase tracking-wider">Assigned TAC</Typography>
              <Typography className="font-medium text-[14px] text-[var(--mui-palette-primary)]">{lead.preferences?.consultantId?.firstName} {lead.preferences?.consultantId?.lastName}</Typography>
            </Box>
          </Box>
        </Box>

        <Box className="bg-[var(--mui-palette-primary)] p-4 md:p-6 rounded-xl shadow-2xl">
          <Typography variant="h6" className="text-[var(--mui-palette-primary)] mb-4 font-medium pb-2">Uploaded Documents</Typography>
          {fetchingDetails ? (
            <Box className="flex flex-col items-center justify-center py-10 gap-2">
              <CircularProgress size={35} />
              <Typography variant="caption" className="text-[var(--mui-palette-primary)] font-medium animate-pulse">Fetching uploaded files...</Typography>
            </Box>
          ) : fullLeadData ? (
        ...
        <Box className="bg-[var(--mui-palette-primary)] p-4 md:p-6 rounded-xl shadow-2xl mt-2">
          <Typography variant="subtitle2" className="font-bold mb-4 uppercase tracking-wider pb-2">Technical Assessment Review</Typography>
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 12 }}>
              <FormControl fullWidth>
                ...
                {fullLeadData ? (
                  <Select
                    displayEmpty size="medium" value={technicalReviewForm.values.expType} label="Experience Type"
                    onChange={(e) => technicalReviewForm.setFieldValue("expType", e.target.value)} name="expType"
                  >
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField fullWidth type="number" label="Achieved Score" name="achievedScore" onChange={technicalReviewForm.handleChange} value={technicalReviewForm.values.achievedScore} error={technicalReviewForm.touched.achievedScore && Boolean(technicalReviewForm.errors.achievedScore)} helperText={technicalReviewForm.touched.achievedScore && technicalReviewForm.errors.achievedScore} />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField fullWidth type="number" name="totalScore" label="Total Score" disabled onChange={technicalReviewForm.handleChange} value={technicalReviewForm.values.totalScore} error={technicalReviewForm.touched.totalScore && Boolean(technicalReviewForm.errors.totalScore)} helperText={technicalReviewForm.touched.totalScore && technicalReviewForm.errors.totalScore} />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField fullWidth name="answered" type="number" label="Answered" onChange={technicalReviewForm.handleChange} value={technicalReviewForm.values.answered} error={technicalReviewForm.touched.answered && Boolean(technicalReviewForm.errors.answered)} helperText={technicalReviewForm.touched.answered && technicalReviewForm.errors.answered} />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField fullWidth name="questions" label="Questions" type="number" onChange={technicalReviewForm.handleChange} value={technicalReviewForm.values.questions} error={technicalReviewForm.touched.questions && Boolean(technicalReviewForm.errors.questions)} helperText={technicalReviewForm.touched.questions && technicalReviewForm.errors.questions} />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField fullWidth name="timeTaken" label="Time Taken" onChange={technicalReviewForm.handleChange} value={technicalReviewForm.values.timeTaken} error={technicalReviewForm.touched.timeTaken && Boolean(technicalReviewForm.errors.timeTaken)} helperText={technicalReviewForm.touched.timeTaken && technicalReviewForm.errors.timeTaken} />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField fullWidth multiline rows={3} name="remarks" label="TAC Head Remarks" placeholder="Provide remarks for approval/rejection..." value={technicalReviewForm.values.remarks} onChange={technicalReviewForm.handleChange} onBlur={technicalReviewForm.handleBlur} error={technicalReviewForm.touched.remarks && Boolean(technicalReviewForm.errors.remarks)} helperText={technicalReviewForm.touched.remarks && technicalReviewForm.errors.remarks} />
            </Grid>
            
            <Grid size={{ xs: 12, md: 12 }}>
              <BreakdownPdfUpload file={technicalReviewForm.values.breakdownPdf} onChange={(f) => technicalReviewForm.setFieldValue("breakdownPdf", f)} />
            </Grid>

            {isPassing && (
              <Grid size={{ xs: 12, md: 12 }}>
                <Box className="p-4 rounded-xl shadow-2xl mt-2 transition-all">
                  <Typography variant="subtitle2" className="mb-3 text-[var(--mui-palette-primary-main)] font-semibold uppercase tracking-wider">Schedule Assessment Slot *</Typography>
                  <TextField
                    type="date" size="small" fullWidth inputProps={{ min: new Date().toISOString().split("T")[0] }} value={selectedDate}
                    onChange={(e) => {
                  ...
                  {selectedDate && (
                    <Box className="mt-4">
                      {fetchingSlots ? (
                        <Box className="flex items-center gap-2"><CircularProgress size={20} /><Typography variant="caption">Fetching available slots...</Typography></Box>
                      ) : slots.length > 0 ? (
                        <Box className="flex flex-wrap gap-2 mt-2">
                          {slots.map((slot: any) => (
                            <Chip
                              key={slot.time} label={slot.time} clickable={slot.available} onClick={() => slot.available && setSelectedSlot(slot)}
                              color={selectedSlot?.time === slot.time ? "primary" : "default"} variant={selectedSlot?.time === slot.time ? "filled" : "outlined"}
      ...
      <DialogActions className="p-5 bg-[var(--mui-palette-primary)]">
        <Button onClick={() => setOpen(false)} className="text-[var(--mui-palette-primary)] font-medium normal-case">Cancel</Button>
        <Button variant="contained" onClick={() => technicalReviewForm.handleSubmit()} className="rounded-lg px-6 normal-case shadow-md font-bold bg-[var(--mui-palette-primary-main)]">
