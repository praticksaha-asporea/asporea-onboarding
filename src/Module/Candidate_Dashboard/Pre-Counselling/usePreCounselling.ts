...
export const usePreCounselling = () => {
  ...
  const [leadUpdated, setLeadUpdated] = useState<boolean>(false)


  const status = usePreCounsellingStatus(leadId, reduxUser, router, leadUpdated);
