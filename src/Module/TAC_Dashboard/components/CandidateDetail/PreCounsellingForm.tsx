...
const PreCounsellingForm: React.FC<PreCounsellingFormProps> = ({
  ...
  const {
    preForm, isPreLocked, setIsPreLocked, previewUrl, isPreviewOpen, setIsPreviewOpen,
    isPdf, isDragging, fileInputRef, handleDragOver, handleDragLeave, handleDrop,
    onFileInputChange, updateAssignmentStatus
      ...
      <form onSubmit={preForm.handleSubmit}>
        ...
        <Stack spacing={3}>
          <Grid container spacing={5}>
            ...
            <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Assigned Consultant" disabled value={consultantId?.firstName ? `${consultantId.firstName} ${consultantId.lastName ?? ""}`.trim() : "—"} /></Grid>
            ...
            {["completed", "rejected", "queued"].includes(preForm?.values?.preStatus) && (
              <>
                <Grid size={{ xs: 12, md: 6 }}>
                ...
                <Grid size={{ xs: 12, md: 6 }} id="resumeFile">
                  ...
                  <Box onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()} className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer transition-all h-[220px] ${isDragging ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:bg-[--mui-palette-secondary-lightOpacity]"}`}>
                    <input ref={fileInputRef} hidden type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={onFileInputChange} disabled={isPreLocked} />
