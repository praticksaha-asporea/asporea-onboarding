...
interface ProgressSidebarProps {
  candidate: any; isFoe: boolean; branchId: any; consultantId: any;
  tacList: any[]; escalateTo: string; setEscalateTo: (val: string) => void; currentUser: any;
}
...
const ProgressSidebar: React.FC<ProgressSidebarProps> = ({ candidate, isFoe, branchId, consultantId, tacList, escalateTo, setEscalateTo, currentUser }) => {
  ...
  return (
    <Card className="p-5 sticky top-6 rounded-xl shadow-2xl">
      ...
      <Box className="mb-4 space-y-2">
        <Typography className="text-[12px] text-[var(--mui-palette-text-primary)]">Branch: <span className="font-semibold">{branchId?.title ?? "—"}</span></Typography>
        <Typography className="text-[12px] text-[var(--mui-palette-text-primary)]">Consultant: <span className="font-semibold">{!isFoe && consultantId?._id === currentUser?.id ? "You" : consultantId?.firstName ? `${consultantId.firstName} ${consultantId.lastName ?? ""}`.trim() : "—"}</span></Typography>
        ...
        <form onSubmit={escalationForm.handleSubmit}>
          <FormControl fullWidth className="mb-4" error={fe("toId")}>
            ...
            <Select label="Escalate" name="toId" value={escalationForm.values.toId} onChange={(e) => { escalationForm.handleChange(e); setEscalateTo(e.target.value as string); }} onBlur={escalationForm.handleBlur}>
              <MenuItem value="">-- Select TAC --</MenuItem>
              {tacList.map((tac) => (<MenuItem key={tac._id} value={tac._id}>{`${tac.firstName} ${tac.lastName ?? ""}`.trim()}</MenuItem>))}
            </Select>
            {fh("toId") && <FormHelperText>{fh("toId")}</FormHelperText>}
          </FormControl>
