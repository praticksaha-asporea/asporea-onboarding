...
const InquiryDetailsForm: React.FC<InquiryDetailsFormProps> = ({ candidate }) => {
  ...
  return (
    <Card className="p-6 rounded-xl shadow-2xl">
      ...
      <form onSubmit={inquiryForm.handleSubmit}>
        <Grid container spacing={5}>
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth name="fullName" label="Full Name" value={inquiryForm.values.fullName} onChange={inquiryForm.handleChange} onBlur={inquiryForm.handleBlur} error={fe("fullName")} helperText={fh("fullName")} /></Grid>
          
          <Grid size={{ xs: 12, md: 6 }} className="flex flex-col -mt-2 justify-center">
            <Typography variant="h6" className="text-[var(--mui-palette-text-secondary)] font-medium mb-2 tracking-wide">Contact Preferences</Typography>
            <Box className="flex flex-wrap gap-2">
               <Chip label="WhatsApp" size="small" className={getChipStyle(!!notifPrefs?.whatsapp)} />
               <Chip label="Email" size="small" className={getChipStyle(!!notifPrefs?.email)} />
               <Chip label="SMS" size="small" className={getChipStyle(!!notifPrefs?.sms)} />
