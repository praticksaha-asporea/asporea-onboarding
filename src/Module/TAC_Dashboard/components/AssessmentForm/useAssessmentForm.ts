...
import { AssessBasicFormValues, AssessmentFormValues, QuestionType } from "@/Types/object.types";
...
export type SignatureField = "candidateSign" | "assessorSign";

export interface SignatureFieldItem {
    label: string;
    field: SignatureField;
}
export const signatureFields: SignatureFieldItem[] = [
    {
...
export type SelectedOptionValue = number | number[] | null;

export interface SelectedOptions {
    [key: number]: SelectedOptionValue;
}

export const useAssessmentForm = ({ selectedCandidate, assessAssign, assessBasicForm, }: { selectedCandidate: CandidateLead, assessAssign: IAssignment, assessBasicForm: FormikProps<AssessBasicFormValues> }) => {
    ...
    const levelScoreMap: Record<string, number> = { L1: 1, L2: 2, L3: 3, L4: 4 };
    ...
    const assessmentForm = useFormik<AssessmentFormValues>({
        initialValues: {
        ...
        onSubmit: async (values) => {
            ...
            formData.append("id", assessAssign?._id.toString());
    ...
    return {
        assessmentStatus, assessmentForm, scoringSections, selectedOptions, setSelectedOptions, languageLevels, setLanguageLevels, levels, skills, languageSections, levelScoreMap, finalTotal, customScores, setCustomScores
    }
}