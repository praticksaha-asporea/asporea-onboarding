...
import { getTacHeadKpis, getTeamOverview } from "@/lib/services/tac_head/dashboard.service";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (authUser.role !== "tac_head") throw new ApiError("TAC Head access required", 403);
        ...
        const [candidates, kpis] = await Promise.all([
            getTeamOverview({ userId: authUser.id }),
            getTacHeadKpis(authUser.id, authUser.role),
        ]);
